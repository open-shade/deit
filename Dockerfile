...
RUN pip3 install ./src/deit && \
    : "Install the model" && \
    python3 -c "from transformers import AutoFeatureExtractor, DeiTForImageClassification; AutoFeatureExtractor.from_pretrained('${MODEL_NAME}'); DeiTForImageClassification('${MODEL_NAME}')" && \
    colcon build
