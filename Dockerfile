...
RUN apt update && \
    ...
    echo "ros2 run deit deit" >> /home/shade/shade_ws/start.sh && \
    chmod +x ./start.sh

COPY . ./src/deit

RUN pip3 install ./src/deit && \
    : "Install the model" && \
    python3 -c "from transformers import AutoFeatureExtractor, DeiTForImageClassification; AutoFeatureExtractor.from_pretrained('${MODEL_NAME}'); DeiTForImageClassification.from_pretrained('${MODEL_NAME}')" && \
    colcon build
