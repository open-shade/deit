...
RUN pip3 install ./src/deit && \
    : "Install the model" && \
    python3 -c "from transformers import AutoFeatureExtractor, ViTForImageClassification; AutoFeatureExtractor.from_pretrained('${MODEL_NAME}'); ViTForImageClassification('${MODEL_NAME}')" && \
    colcon build
